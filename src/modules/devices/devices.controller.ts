...
export class DevicesController {
  ...
  async registerDevice(@Request() req, @Body() dto: RegisterDeviceDto) {
    const device = await this.devicesService.registerDevice(req.user.sub, dto);
  ...
  async deactivateDevice(@Request() req, @Param('deviceId') deviceId: string) {
    await this.devicesService.deactivateDevice(deviceId, req.user.sub);
    return { success: true };
  }
  ...
  async getPreferences(@Request() req) {
    return this.devicesService.getPreferences(req.user.sub);
  }
  ...
  async updatePreferences(
    ...
    return this.devicesService.updatePreferences(req.user.sub, dto);
  }
}
